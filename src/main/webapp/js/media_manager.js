...
export class MediaManager {
    ...
    initLocalStream(streamId) {
        ...
        if (
            ...
            return this.openStream(this.mediaConstraints, streamId);
        } else if (
    ...
    async setDesktopWithCameraSource(stream, streamId, onEndedCallback) {
        ...
        if (this.localStream == null) {
            ...
            promise = this.updateVideoTrack(canvasStream, streamId, onEndedCallback, null);
        }
    ...
    async prepareStreamTracks(mediaConstraints, audioConstraint, deviceStream, streamId) {
        ...
        if (this.publishMode == "screen") {
            await this.updateVideoTrack(deviceStream, streamId, onEnded, true);
        } else if (this.publishMode == "screen+camera") {
            await this.setDesktopWithCameraSource(deviceStream, streamId, onEnded);
        }
        ...
        if (audioConstraint) {
            ...
            if (this.publishMode !== "camera") {
                await this.updateAudioTrack(audioStream, streamId, null);
     ...
     * @param {*} func : callback on success. The stream which is got, is passed as parameter to this function
     */
    async getDisplayMedia(mediaConstraints, func) {
        try {
            return await navigator.mediaDevices.getDisplayMedia(mediaConstraints).then((stream) => {
     ...
     * @param {*} streamId
     */
    async switchMediaSource(streamId, sourceId) {
        this.publishMode = "media";
        ...
        const stream = await this.getUserMedia(m);
    ...
    async applyConstraints(newConstraints) {
        ...
        if (mediaConstraints.audio !== undefined && this.publishStreamId) {
            //just give the audio constraints not to get video stream
