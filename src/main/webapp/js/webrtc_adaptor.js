...
export class WebRTCAdaptor {
    ...
    async initialize() {
        if (
            ...
            return await this.mediaManager
                .initialize()
                .then(() => {
                    this.initPlugins();
     ...
     */
    async publish(streamId, token, subscriberId, subscriberCode, streamName, mainTrack, metaData) {
    ...
    sendPublishCommand(
        ...
        var jsCmd = {
            ...
            token,
            subscriberId:
                typeof subscriberId !== undefined && subscriberId != null ? subscriberId : "",
            subscriberCode:
    ...
    play(streamId, token, roomId, enableTracks, subscriberId, subscriberCode, metaData) {
        ...
        const jsCmd = {
            ...
            trackList:
                typeof enableTracks !== undefined && enableTracks != null ? enableTracks : [],
            subscriberId:
                typeof subscriberId !== undefined && subscriberId != null ? subscriberId : "",
            subscriberCode:
    ...
    async getStats(streamId) {
        const connection = this.remotePeerConnection[streamId];
        if (!connection) {
    ...
    /**
     * Called to start a periodic timer to get statistics periodically (1 seconds) for a specific stream.
     *
     * @param {string} streamId: unique id for the stream
     * @param {number} interval: stats interval in milliseconds. Default is 1000 ms.
     */
    enableStats(streamId, interval = 1000) {
    ...
    closeWebSocket() {
        ...
        // No adaptor exists
        if (this.webSocketAdaptor) this.webSocketAdaptor.close();
    }
    ...
    sendData(streamId, data) {
        ...
        if (this.remotePeerConnection[streamId] !== undefined) {
            ...
            Logger.warn(
                "Send data is called for undefined peer connection with stream id: " + streamId,
            );
        }
    }
     ...
     * @returns
     */
    applyConstraints(newConstaints, streamId) {
        this.mediaManager.applyConstraints(newConstaints, streamId);
    }
